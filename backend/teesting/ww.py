...
LOG_FILE = "log_testing_full_workflow_e2e_revisi.txt"
...
def konsumen_checkout(token, id_titik_ambil):
    ...
    return res.json().get("checkout", {}).get("_id"), res.json().get("checkout", {}).get("total_harga", 0)
...
def konsumen_create_payment(token, order_id, jumlah):
    ...
    res = requests.post(f"{BASE}/payments", headers={"Authorization": f"Bearer {token}"}, json=data)
    log("KONSUMEN: CREATE PAYMENT", res)
    return res.json().get("payment", {}).get("_id")

def konsumen_upload_bukti(token, payment_id, bukti_path="bukti_e2e.jpg"):
    if not os.path.exists(bukti_path):
        with open(bukti_path, "wb") as f: f.write(b"\x89PNG\r\n\x1a\n")
    files = {"bukti_pembayaran": open(bukti_path, "rb")}
    res = requests.post(f"{BASE}/payments/{payment_id}/upload-bukti", headers={"Authorization": f"Bearer {token}"}, files=files)
    log("KONSUMEN: UPLOAD BUKTI PEMBAYARAN", res)

def admin_approve_payment(token, payment_id):
    res = requests.patch(f"{BASE}/payments/{payment_id}/verify", headers={"Authorization": f"Bearer {token}"}, json={"status": "berhasil"})
    log("ADMIN: APPROVE PAYMENT", res)
...
def main():
    ...
    # Step 3: Konsumen - Register, login, add cart, update cart, checkout
    ...
    id_checkout, total_harga = konsumen_checkout(token_konsumen, id_titik_ambil)

    # Step 4: Admin - Approve checkout
    admin_approve_checkout(token_admin, id_checkout)

    # Step 5: Setelah approve, ambil order_id dari /orders
    order_id = admin_get_orders(token_admin)

    # Step 6: Konsumen create payment untuk order_id
    id_payment = konsumen_create_payment(token_konsumen, order_id, total_harga)
    konsumen_upload_bukti(token_konsumen, id_payment)

    # Step 7: Admin approve payment
    admin_approve_payment(token_admin, id_payment)

    # Step 8: Admin add logistics
    admin_add_logistics(token_admin, order_id, id_petani, id_titik_ambil)

    # Step 9: Petugas - Register, login, pickup verification
    token_petugas, _, _ = register_login("petugas")
    petugas_pickup_verification(token_petugas, order_id, id_konsumen, id_titik_ambil)

    # Step 10: Konsumen - delete profile (contoh DELETE)
