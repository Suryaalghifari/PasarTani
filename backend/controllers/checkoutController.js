...
exports.uploadBuktiPembayaran = async (req, res) => {
  try {
    ...
    if (!file) {
      return res
        .status(400)
        .json({ message: "Bukti pembayaran wajib di-upload." });
    }
    ...
    if (!checkout)
      return res
        .status(404)
        .json({ message: "Data checkout tidak ditemukan." });
  ...
  try {
    ...
    if (status_checkout === "paid" && !existingOrder) {
      await Order.create({
        ...
        status_pembayaran: "paid", // <--- INI YANG HARUS DITAMBAHKAN!
    ...
    res.json({
      message: `Status pembayaran diubah menjadi ${status_checkout}`,
      checkout,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
